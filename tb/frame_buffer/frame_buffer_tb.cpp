...
SC_MODULE(top_tb) {
    ...
    void debayer2rgb(vector<uint8_t> &mem, int cnt) {
        ...
        Mat m = Mat(VRES, HRES, CV_8UC1, mem.data());
        Mat m_debayer;
    ...
    Mat savePicture(vector<uint8_t> &mem, int cnt) {
        ...
        Mat m = Mat(VRES, HRES, CV_8UC3, mem.data());
        cout << "memory size: " << mem.size() << endl;
        cout << "Mat size: " << m.size() << endl;
...
private:
    ...
    void sequencer() {
        while (true) {//forever begin
            wait(clk_wr.posedge_event()); // @(posedge clk)
    ...
    void monitor() {
        ...
        while (true) {      // forever begin
            wait(clk_rd.posedge_event()); // @(posedge clk)
            ++clk_cnt;
            ...
            if (r1_vsync && !o_vsync) {
                ...
                savePicture(memory, frame_cnt);
                memset(memory.data(), 0, VRES*HRES*3);
                x_cnt = 0;
                y_cnt = 0;
            }
            ...
            if (o_valid) {
                memcpy(memory.data() + (y_cnt)*(3*HRES) + x_cnt*(2*3), &o_rgb.read(), 3*2);
                cout << "y count: " << y_cnt << ". ";
    ...
    void ref_monitor() {
        ...
        while (true) {      // forever begin
            ...
            if (r1_vsync && !ref_vs) {
                ...
                memset(ref_memory.data(), 0, VRES*HRES);
                x_cnt = 0;
                y_cnt = 0;
            }
            ...
            if (ref_valid) {
                memcpy(ref_memory.data() + (y_cnt)*(HRES) + x_cnt*(4), &ref_raw.read(), 4);
...
public:
    ...
    top_tb (
        ...
        std::unique_ptr<Vframe_buffer_tb> &dut,
        size_t max_vsync_cnt
    ) 
    :sc_module(name)
    ,clk_wr("clk_wr", 10, SC_NS, 0.5, 3, SC_NS, true)
    ,clk_rd("clk_rd", 6.4, SC_NS, 0.5, 3, SC_NS, true)
    {    
        ...
        dut->ref_raw(ref_raw);
        rstn = false;
        memory.resize(HRES*VRES*3);
        ref_memory.resize(HRES*VRES);
...
int sc_main(int argc, char* argv[]) {
    ...
    std::unique_ptr<Vframe_buffer_tb> dut(new Vframe_buffer_tb("dut"));
